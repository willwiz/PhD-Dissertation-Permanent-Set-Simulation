...
%\item Any applied deformation to the tissue referenced to the unloaded configuration is defined as $\mathbf{F}$
\end{enumerate}
...
%Although it is a unloaded reference configuration, it is also the stress free reference configuration for a fraction of the exogenously crosslinked matrix like the evolving loaded configuration $\Omega(s)$.
...
Thus the right Cauchy Green tensor when referenced to the strain history $\Omega(\hat{s})$ is given by $\mathbf{\bar{C}}(\hat{s}) = \mathbf{\bar{F}}(\hat{s})^\mathsf{T} \cdot \mathbf{\bar{F}}(\hat{s})$. This also has the following relation to the applied deformation $\mathbf{F}$,
...
\subsection{Extension to the exogenously crosslinked matrix model for changes in reference configuration}
	...
	We start by extending the exogenously crosslinked matrix model from our previous model \cite{sacks_novel_2016}. The strain energy is a modified form of the Yeoh model which exhibit a more linear stress-strain relation,
\begin{equation}
\begin{split}
...
\subsubsection{Extension of the exogenously crosslinked matrix for the permanent set effect}
...
\end{figure}
	...
	Next, we developed the model form for the exogenously crosslinked matrix after permanent set. This approach is based on the work by Rajagopal and Wineman \cite{rajagopal_constitutive_1992}, where we assume that the response of the full exogenously crosslinked matrix to be 
...
Now that the model form for the exogenously crosslinked matrix under permanent set is established, we need to consider how the collagen fiber architecture is convected by the change in reference configuration. 
...
\end{gathered}
\end{equation}
%where the change in orientation $\theta_0$ from the 0-cycle state to $\theta_1$ after push forward $_0^1\mathbf{F}$ is
%\begin{equation}
%\theta_1\left(\prescript{1}{0}{\mathbf{F}}\right) = \arctan\left( \frac{\prescript{1}{0}{F}_{21} \sin\theta_0 + \prescript{1}{0}{F}_{22} \cos\theta_0}{\prescript{1}{0}{F}_{11} \sin\theta_0 + \prescript{1}{0}{F}_{12} \cos\theta_0}\right).
%\end{equation}
...
%are determined from the old bounds $(\lambda_lb,\lambda_ub)$ 
%\begin{equation}
%\prescript{}{1}{\lambda}_\mathrm{lb} = \frac{\lambda_lb}{\prescript{1}{0}{\lambda}(\theta)}, \qquad \prescript{}{1}{\lambda}_\mathrm{ub} = \frac{\lambda_ub}{\prescript{1}{0}{\lambda}(\theta)},
%\end{equation} 
%and the shape parameters of the recruitment distribution $(\gamma_0,\gamma_1)$, which is determined from the mean $(\mu_\mathrm{D})$ and standard deviation $(\sigma_\mathrm{D})$ as
%\begin{equation}
%\begin{gathered}
%\bar{\mu}=\frac{\mu_D - \lambda_\mathrm{lb}}{\lambda_\mathrm{ub} - \lambda_\mathrm{lb}}, \qquad \bar{\sigma}=\frac{\sigma_D}{\lambda_\mathrm{ub} - \lambda_\mathrm{lb}},\\
%\gamma_0 = \left( \frac{1-\bar{\mu}}{\bar{\sigma}^2} - \frac{1}{\bar{\mu}}\right) \bar{\mu}^2, \qquad \gamma_1 = \gamma_0 \left( \frac{1}{\bar{\mu}} -1\right).
%\end{gathered}
%\end{equation}
%\begin{equation}
%\begin{gathered}
%\bar{\mu}=\frac{\mu_D - \prescript{}{0}{\lambda}_\mathrm{lb}}{\prescript{}{0}{\lambda}_\mathrm{ub} - \prescript{}{0}{\lambda}_\mathrm{lb}}, \qquad \bar{\sigma}=\frac{\sigma_D}{\prescript{}{0}{\lambda}_\mathrm{ub} - \prescript{}{0}{\lambda}_\mathrm{lb}},\\
%\gamma_0 = \left( \frac{1-\bar{\mu}}{\bar{sigma}^2} - \frac{1}{\bar{\mu}}\right) \bar{\mu}^2, \qquad \gamma_1 = \gamma_0 \left( \frac{1}{\bar{\mu}} -1\right).
%\end{gathered}
%\end{equation}
...
\subsection{Futher considerations of the permanent set mechanism: using the convected collagen fiber architecture to predict the mechanical response after permanent set induced deformation due to cyclic loading}
	...
	We can validate the mechanisms we proposed for permanent set by performing an initial analysis on predicting the mechanical response after cyclic loading using only the convection of the collagen fiber architecture. 
...
%\begin{equation} \label{eq:fullEXLmodel}
%\begin{split}
%\mathbf{S}\left(k , \mathbf{F}_\mathrm{PS}, \mathbf{A}(\hat{s}), \mathbf{C}\right) =& \phi_C \mu_C \int\displaylimits_\theta \Gamma_1(\mathbf{F}_{\mathrm{PS}}, \theta)\left\lbrace 
%\int\displaylimits_1^{\lambda_\theta} \frac{D_1\left( \mathbf{F}_{\mathrm{PS}}, x \right)}{x} \left( \frac{1}{x}- \frac{1}{\lambda_\theta}\right) \mathrm{d}x \right\rbrace \mathbf{n}(\theta)\otimes\mathbf{n}(\theta) \mathrm{d}\theta  \\
%+& \phi_C \mu_\mathrm{int} \int\displaylimits_\alpha \int\displaylimits_\beta \Gamma_1 \left(\mathbf{F}_\mathrm{PS}, \alpha \right) \Gamma_1 \left(\mathbf{F}_\mathrm{PS},  \beta \right) \\
%\times&\left[ \left\lbrace 
%\int\displaylimits_1^{\lambda_\alpha} \int\displaylimits_1^{\lambda_\beta} 
%\frac{2 \lambda_\beta D_1(\mathbf{F}_\mathrm{PS}, x_\alpha) D_1(\mathbf{F}_\mathrm{PS}, x_\beta)}{x_\alpha x_\beta} 
%\left( \frac{\lambda_\alpha}{x_\alpha} \frac{\lambda_\beta}{x_\beta} - 1\right) \mathrm{d}x_\alpha \, \mathrm{d}x_\beta \right.\right. \\
...
%\right. \right. \\
%+&\left. \left. \int\displaylimits_1^{\lambda_\alpha} D_1(\mathbf{F}_\mathrm{PS}, x_\alpha) \left( \frac{\lambda_\alpha}{x_\alpha} -1  \right)^2 \mathrm{d}x_\alpha \right\rbrace \frac{\mathbf{n}_\beta \otimes \mathbf{n}_\beta}{\lambda_\beta}  \right] \mathrm{d}\alpha \, \mathrm{d}\beta \\
%+&\phi_m \mu_m \left[ \vphantom{\int\displaylimits_0^s} e^{-k s}  \left(\left( \bar{I_1} (\mathbf{F}_\mathrm{PS}, \mathbf{A}(0)) - 3\right)^{\alpha - 1} + r \left( \bar{I_1} (\mathbf{F}_\mathrm{PS}, \mathbf{A}(0)) - 3\right)^{\beta - 1}\right)  \right.\\
%\times& \left( \mathbf{\tilde{B}}(\mathbf{F}_\mathrm{PS}, \mathbf{A}(0)) - \tilde{B}_{33}^{-1}(\mathbf{F}_\mathrm{PS}, \mathbf{A}(0))C_{33}\mathbf{C}^{-1}\right) \\
%+& \int\displaylimits_0^s \phi_m k  e^{-k (s - \hat{s})} \left(\left( \bar{I_1} (\mathbf{F}_\mathrm{PS}, \mathbf{A}(\hat{s})) - 3\right)^{\alpha - 1} + r \left( \bar{I_1} (\mathbf{F}_\mathrm{PS}, \mathbf{A}(\hat{s})) - 3\right)^{\beta - 1}\right) \\
%\times& \left. \vphantom{\int\displaylimits_-^s} \left( \mathbf{\tilde{B}}(\mathbf{F}_\mathrm{PS}, \mathbf{A}(\hat{s})) - \tilde{B}_{33}^{-1}(\mathbf{F}_\mathrm{PS}, \mathbf{A}(\hat{s}))C_{33}\mathbf{C}^{-1}\right) \mathrm{d}\hat{s}\right].
%\end{split}
%\end{equation}
